...
def personal_anal():
    ...
    for cnt, mat in enumerate(pgc_data['match_id']):
        ...
        for player in players:
            # 이미 저장된 데이터는 생략
            if os.path.isfile(
                    "C:/Users/HC/PycharmProjects/pubgProject/MatchList/datas/pgc/Anal/" + mat + "_" + player + ".csv"):
        ...
        for vehicle in vehicles:
            if vehicle['vehicle']['vehicle_type'] != 'WheeledVehicle' or vehicle['character']['name'] in firstVehicle.keys() or vehicle['character']['name'][:vehicle['character']['name'].find('_')] in used_teamId:
                continue
            else:
                firstVehicle[vehicle['character']['name'][:vehicle['character']['name'].find('_')]] = (pd.to_timedelta(vehicle.timestamp[vehicle.timestamp.find('T') + 1:-1]) - startTime).total_seconds()
                used_teamId.append(vehicle['character']['name'][:vehicle['character']['name'].find('_')])
        vehicle_df = pd.DataFrame(list(firstVehicle.items()), columns=["teamName", "time"])
        firstVehicle_df = pd.concat([firstVehicle_df, vehicle_df], axis=1, sort=False)
