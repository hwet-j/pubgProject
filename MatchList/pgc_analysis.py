...
def pro_anal():
    ...
    for cnt, mat in enumerate(pgc_data['match_id']):
        ...
        # print(telemetry.damage_taken())
        # print(telemetry.damage_done())
        # print(len(telemetry.damage_done()))
        # print(telemetry.player_damages())
        # print(len(telemetry.player_damages()))
        # print(telemetry.filter_by("log_player_take_damage"))
        ...
        for key, value in attacker_victim_data.items():
            for row in value:
                attacker_victim_df = attacker_victim_df.append({"time":row[0],
                                           "attacker":row[1],
                                           "victim":row[2],
                                           "damage":row[3],
                                           "hit_type":row[4],
        ...
        attackData = pd.DataFrame(attackData, columns=['attacker', 'time'])
        attackData['attack_teamName'] = attackData['attacker'].str.extract(r'([0-9A-Za-z]+)\_')  # 팀명 추출
        ...
        for team in attackData['attack_teamName'].unique():
            try:
                teamCount[team] += 1
            except KeyError:
                teamCount[team] = 1
    ...
    teamAttack_df['phasePercent'] = (teamAttack_df['countMean'] / teamAttack_df['countMean'].sum(level=0)).round(4) * 100
    print(teamAttack_df)
