...
def pro_anal():
    ...
    for cnt, mat in enumerate(pgc_data['match_id']):
        # 이미 저장된 데이터는 생략
        if os.path.isfile("C:/Users/HC/PycharmProjects/pubgProject/MatchList/datas/pgc/Anal/" + mat + ".csv"):
        ...
        for player in players:
            ...
            for phase in phases:  # 자기장 별 데이터 분리
                ...
                blueBorder_diff = (blues[pre:phase, 4] - np.sqrt(np.square(cur_blue_xDiff) + np.square(cur_blue_yDiff)))  / blues[pre:phase, 4]
